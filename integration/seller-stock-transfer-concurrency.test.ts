...
const DESTINATION_LOCATION_ID = "50000000-0000-4000-8000-000000000003";
...
async function adjustTo(local: Config, admin: string, locationId: string, target: number) {
  ...
  expect(await rpc(local, admin, "adjust_stock", {
    ...
    p_reason: "Preparar corrida entre venda e transferência", p_idempotency_key: `seller-transfer-adjust:${randomUUID()}`,
    p_correlation_id: randomUUID(),
  })).toMatchObject({ ok: true });
}

it("serializa o aceite da transferência contra a reserva da última unidade", async () => {
  const local = config();
  const [admin, sourceSeller, destinationSeller] = await Promise.all([
    login(local, "admin.teste@institutojef.org.br", "Admin123!"),
    login(local, "vendedor.teste@institutojef.org.br", "Vendedor123!"),
    login(local, "vendedor.destino@institutojef.org.br", "VendedorDestino123!"),
  ]);
  ...
  const requested = await rpc(local, destinationSeller, "request_seller_stock_transfer", {
    p_from_location_id: SOURCE_LOCATION_ID, p_product_id: PRODUCT_ID, p_quantity: 1,
    p_reason: "Última unidade para venda concorrente", p_idempotency_key: `seller-transfer-request:${randomUUID()}`,
    p_correlation_id: randomUUID(),
  });
  expect(requested).toMatchObject({ ok: true });
  const requestId = String((requested as Extract<Outcome, { ok: true }>).data.request_id);
  const reservationOrigin = `seller-transfer-race:${randomUUID()}`;

  const raced = await Promise.all([
    rpc(local, sourceSeller, "resolve_seller_stock_transfer", {
      p_request_id: requestId, p_action: "ACCEPT", p_reason: "Saldo separado para entrega",
      p_idempotency_key: `seller-transfer-accept:${randomUUID()}`, p_correlation_id: randomUUID(),
    }),
    rpc(local, admin, "reserve_stock", {
      ...
      p_idempotency_key: `seller-transfer-reserve:${randomUUID()}`, p_correlation_id: randomUUID(),
    }),
  ]);
  ...
  if ("movement_id" in winner.data) {
    ...
    expect(await rpc(local, admin, "reverse_stock_movement", {
      p_movement_id: winner.data.movement_id, p_reason: "Restaurar fixture após corrida de transferência",
      p_idempotency_key: `seller-transfer-reverse:${randomUUID()}`, p_correlation_id: randomUUID(),
    })).toMatchObject({ ok: true });
  } else {
    ...
    expect(await rpc(local, admin, "release_stock_reservation", {
      ...
      p_idempotency_key: `seller-transfer-release:${randomUUID()}`, p_correlation_id: randomUUID(),
    })).toMatchObject({ ok: true });
    expect(await rpc(local, destinationSeller, "resolve_seller_stock_transfer", {
      p_request_id: requestId, p_action: "CANCEL", p_reason: "Reserva concorrente venceu a disputa",
      p_idempotency_key: `seller-transfer-cancel:${randomUUID()}`, p_correlation_id: randomUUID(),
    })).toMatchObject({ ok: true });
  }
