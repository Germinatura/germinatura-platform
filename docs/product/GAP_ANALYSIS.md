...
| AUTH-003 Acesso institucional por código | EM IMPLEMENTAÇÃO — a branch `feat/institutional-auth` contém OTP, domínio exato imposto pelo banco, rate limit persistente e E2E com Mailpit | Template e capacidade real de entrega de e-mail ainda precisam ser provados em staging | P0 | Supabase Auth, SMTP | Alto | Validar deploy/configuração hospedada e envio para conta fictícia autorizada antes de marcar DONE |
| AUTH-004 Papel base e ativação do vendedor | EM IMPLEMENTAÇÃO — criação mantém apenas `CONSUMIDOR`; RPC administrativa troca papéis cumulativos, ativa/inativa e audita; PDV verifica perfil ativo | Interface administrativa completa fica na fatia operacional; evidência remota ainda pendente | P0 | AUTH-003, RBAC, auditoria | Alto | Validar abuso e revogação em staging e integrar à UI administrativa |
