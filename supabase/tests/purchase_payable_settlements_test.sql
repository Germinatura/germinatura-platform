...
select throws_ok($$select public.settle_purchase_payable(gen_random_uuid(),100,(now() at time zone 'America/Sao_Paulo')::date,'PIX','REF-1','Tentativa indevida','payable-denied',gen_random_uuid())$$,'42501','FINANCE_MANAGE_FORBIDDEN','consumer cannot settle a payable');
...
create temp table payable_order as select public.create_purchase_order(
  (select (result->>'id')::uuid from payable_supplier),(now() at time zone 'America/Sao_Paulo')::date,null,250,50,'PIX após entrega',null,null,
...
create temp table payable_receipt as select public.receive_purchase_order_item(
  (select (result->>'id')::uuid from payable_order),(select id from payable_item),2,(now() at time zone 'America/Sao_Paulo')::date,
...
select throws_ok($$select public.settle_purchase_payable((select id from payable_target),100,(now() at time zone 'America/Sao_Paulo')::date-1,'PIX','BEFORE','Pagamento anterior','payable-before',gen_random_uuid())$$,'22023','INVALID_PAYABLE_SETTLEMENT','settlement cannot precede the receipt');
select throws_ok($$select public.settle_purchase_payable((select id from payable_target),100,(now() at time zone 'America/Sao_Paulo')::date+1,'PIX','FUTURE','Data futura','payable-future',gen_random_uuid())$$,'22023','INVALID_PAYABLE_SETTLEMENT','future settlement date rejected');
...
create temp table partial_settlement as select public.settle_purchase_payable(
  (select id from payable_target),600,(now() at time zone 'America/Sao_Paulo')::date,'PIX PicPay Empresas','PIX-001','Pagamento parcial conferido','payable-partial',gen_random_uuid()) result;
...
select results_eq($$select public.settle_purchase_payable((select id from payable_target),600,(now() at time zone 'America/Sao_Paulo')::date,'PIX PicPay Empresas','PIX-001','Pagamento parcial conferido','payable-partial',gen_random_uuid())$$,$$select result from partial_settlement$$,'settlement replay returns original result');
...
select throws_ok($$select public.settle_purchase_payable((select id from payable_target),951,(now() at time zone 'America/Sao_Paulo')::date,'PIX','PIX-OVER','Pagamento excedente','payable-over',gen_random_uuid())$$,'P0001','PURCHASE_PAYABLE_AMOUNT_EXCEEDED','overpayment rejected under lock');
create temp table final_settlement as select public.settle_purchase_payable(
  (select id from payable_target),950,(now() at time zone 'America/Sao_Paulo')::date,'PIX PicPay Empresas','PIX-002','Pagamento final conferido','payable-final',gen_random_uuid()) result;
...
select throws_ok($$select public.settle_purchase_payable((select id from payable_target),1,(now() at time zone 'America/Sao_Paulo')::date,'PIX','PIX-LATE','Pagamento adicional','payable-late',gen_random_uuid())$$,'P0001','PURCHASE_PAYABLE_ALREADY_SETTLED','settled obligation rejects another payment');
create temp table payable_reversal as select public.reverse_purchase_payable_settlement(
  (select (result->>'id')::uuid from partial_settlement),(now() at time zone 'America/Sao_Paulo')::date,'Pagamento lançado na conta errada','payable-reverse',gen_random_uuid()) result;
...
select results_eq($$select public.reverse_purchase_payable_settlement((select (result->>'id')::uuid from partial_settlement),(now() at time zone 'America/Sao_Paulo')::date,'Pagamento lançado na conta errada','payable-reverse',gen_random_uuid())$$,$$select result from payable_reversal$$,'reversal replay returns original result');
select throws_ok($$select public.reverse_purchase_payable_settlement((select (result->>'id')::uuid from partial_settlement),(now() at time zone 'America/Sao_Paulo')::date,'Segunda tentativa de reversão','payable-reverse-twice',gen_random_uuid())$$,'P0001','PAYABLE_SETTLEMENT_ALREADY_REVERSED','settlement cannot be reversed twice');
reset role;
