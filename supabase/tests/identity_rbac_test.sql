...
select results_eq('select count(*)::bigint from public.permissions', array[25::bigint], 'permission catalog includes inventory and procurement capabilities');
select results_eq(
...
select results_eq($$select count(*)::bigint from public.role_permissions rp join public.roles r on r.id = rp.role_id where r.key = 'ADMIN'$$, array[25::bigint], 'administrator receives every permission');
