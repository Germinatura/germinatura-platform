...
O repositório contém a fundação autenticada, os health checks do Portal e do PDV, uma primitive compartilhada de dinheiro em centavos, pricing-base puro para subtotais e total do carrinho e a primeira regra promocional `QUANTIDADE_PRECO`, além da fundação persistente de idempotência e contratos neutros de pagamento que falham de forma fechada. Promoções persistem escopo de produto, canais, prioridade, cumulatividade, vigência `[)`, limites e a regra em centavos; uma visão com RLS expõe somente regras atuais e publicáveis, sem permitir escrita pela Data API. O catálogo persiste categorias, produtos e histórico de preços em centavos com RLS e expõe `GET /api/v1/catalog/products`, uma leitura pública anônima, limitada e paginada; ainda não há tela de catálogo. O inventário possui localizações central/vendedor, saldos protegidos, ledger transacional para ajustes, transferências e reversões e reservas de estoque atômicas com liberação/expiração idempotentes. Auditoria e outbox são gravadas no mesmo commit; a outbox ainda não possui consumidor operacional. Resolução server-side conjunta de preços e promoções, API de cotação, venda/checkout, reserva comercial com snapshot de preço, integrações de pagamento, rifas, financeiro, comunidade e notificações ainda não estão implementados. As telas antigas desses domínios foram retiradas da navegação e suas APIs foram removidas para não aparentarem funcionalidade sem um modelo transacional v2.
