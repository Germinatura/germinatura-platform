...
export function SaleWorkspace({ user }: { user: PdvSessionUser }) {
  const [view, setView] = useState<"sale" | "transfers" | "returns" | "closeout">("sale");
      ...
      <div className="mx-auto max-w-[var(--g-content-wide)] px-4 py-6 md:px-6 md:py-8">
        <div className="mb-6 flex items-start justify-between gap-4">
          <div><Badge tone="success"><Check className="size-3.5" /> Acesso autorizado</Badge>
            <h1 className="mt-3 text-2xl font-bold tracking-tight md:text-3xl">{view === "closeout" ? "Fechamento" : view === "transfers" ? "Transferências entre vendedores" : view === "returns" ? "Devoluções à central" : { catalog: "Nova venda", review: "Revisar venda", payment: "Confirmar pagamento", success: "Venda concluída" }[step]}</h1>
            <p className="mt-1 text-sm text-[var(--g-text-secondary)]">{view === "closeout" ? "Confira o período, conte o estoque físico e registre divergências." : view === "transfers" ? "Solicite produtos e aceite apenas movimentações destinadas ao seu estoque." : view === "returns" ? "Envie produtos de volta e acompanhe a conferência física da central." : { catalog: "Selecione os produtos e quantidades para começar.", review: "Confira os valores recalculados pelo servidor antes de cobrar.", payment: "Registre somente depois de confirmar o recebimento fora do sistema.", success: "Pagamento, estoque e financeiro foram registrados juntos." }[step]}</p>
          </div>
    ...
    <section aria-label="Catálogo">
      ...
      {props.catalog.length === 0 ? <EmptyState icon={PackageSearch} title="Nenhum produto encontrado" description="Tente outro termo ou peça a um administrador para conferir o catálogo e o estoque deste local." />
        : <div className="grid gap-4 sm:grid-cols-2 xl:grid-cols-3">{props.catalog.map((product) => {
          ...
          return <Card key={product.id} className="flex min-h-52 flex-col overflow-hidden p-5">
            {product.images[0] ? <div role="img" aria-label={product.images[0].altText} className="-mx-5 -mt-5 mb-4 h-28 bg-cover bg-center" style={{ backgroundImage: `url(${JSON.stringify(product.images[0].publicUrl)})` }} /> : null}
