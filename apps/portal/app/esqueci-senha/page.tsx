...
export default function EsqueciSenhaPage() {
  ...
  async function submit(event: React.FormEvent) {
    ...
    try {
      ...
      if (!parsed.success) throw new Error(codeSent ? "Informe o código de 6 a 10 dígitos" : "Informe usuário ou e-mail válido");
    ...
    <main className="flex min-h-screen items-center justify-center bg-background-light p-4">
      <section className="w-full max-w-md rounded-3xl bg-white p-8 shadow-xl">
        ...
        <form onSubmit={submit} className="mt-6 space-y-5">
          ...
          {codeSent && <label className="grid gap-2 font-bold text-slate-700">Código de 6 a 10 dígitos<input required inputMode="numeric" autoComplete="one-time-code" minLength={6} maxLength={10} value={token} onChange={(event) => setToken(event.target.value.replace(/\D/g, ""))} className="rounded-2xl border border-slate-200 bg-slate-50 p-4 text-center text-xl tracking-[.3em]" /></label>}
