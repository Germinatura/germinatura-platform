...
import { StockReturnManagement } from "@/components/admin/StockReturnManagement";
import { StockLossManagement } from "@/components/admin/StockLossManagement";
...
export default async function InventoryAdminPage({ searchParams }: { searchParams: Promise<{ q?: string }> }) {
  ...
  return <div className="px-4 py-8 sm:px-6 lg:px-8 lg:py-10"><div className="mx-auto max-w-[var(--g-content-standard)] space-y-6">
    ...
    {!unavailable && <StockDistributionForm
      locations={locations.filter((location) => location.active).map((location) => ({ id: location.id, name: location.name, locationType: location.location_type }))}
      balances={balances.map((balance) => ({ locationId: balance.location_id, productId: balance.product_id, availableQuantity: balance.available_quantity }))}
      products={products.filter((product) => product.active).map((product) => ({ id: product.id, sku: product.sku, name: product.name }))}
    />}
    <StockReturnManagement />
    <StockLossManagement />
