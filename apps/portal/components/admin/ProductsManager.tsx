...
import { type CatalogProduct, saveCatalogProductResponseSchema, saveCatalogProductSchema } from "@germinatura/contracts";
    ...
    <Card className="p-5">
      ...
      {products.length === 0 ? <p className="mt-6 text-[var(--g-text-secondary)]">Nenhum produto corresponde à busca. Cadastre um produto pelo formulário.</p> :
        <ul className="mt-4 divide-y divide-[var(--g-border-subtle)]">{products.map((item) => <li key={item.id} className="flex flex-wrap items-center justify-between gap-3 py-4">
          ...
          <Button type="button" variant="secondary" aria-label={`Editar produto ${item.name}`} onClick={() => { setSelected(item.id); setNotice(""); }}>Editar</Button>
        </li>)}</ul>}
    </Card>
...
function ProductForm({ product, categories, onNew, onSaved }: { product?: CatalogProduct; categories: CatalogCategoryOption[]; onNew: () => void; onSaved: () => void }) {
  ...
  return <Card className="p-5">
    ...
    <form onSubmit={submit} className="mt-5 space-y-4" aria-label={product ? "Editar produto" : "Novo produto"}>
      <fieldset disabled={saving || activeCategories.length === 0} className="space-y-4">
        ...
        <div className="flex flex-wrap gap-3"><Button type="submit" loading={saving}>{product ? "Salvar produto" : "Criar produto"}</Button>{product && <Button type="button" variant="secondary" onClick={onNew}>Novo produto</Button>}</div>
      </fieldset>
